...
const fetchAllOrders = async () => {
  ...
  try {
    ...
    if (response && response.data) {
      ...
      console.log("모든 주문 목록 로드 성공:", salesData.value);
    } else {
...
const fetchChartData = async () => {
  ...
  try {
    ...
    console.log(`API 요청: startDate=${startDate}, endDate=${endDate}`);
    ...
    if (response && response.data) {
      ...
      console.log("차트 데이터 로드 성공:", chartData.value);
    } else {
      console.error("API 응답 오류:", response);
      chartData.value = [];
    }
  ...
  if (rangeType === 'hour') {
    ...
    apiData.forEach(item => {
      ...
      item.oneTimeResponseList.forEach(sale => {
        // 판매 금액을 24시간에 임의로 분배 (실제 구현에서는 API에서 시간별 데이터를 받아야 함)
        const salePrice = sale.salePrice;
        // 임의의 시간대에 할당 (실제 구현에서는 API에서 시간별 데이터를 받아야 함)
        const randomHour = Math.floor(Math.random() * 24);
        const hour = String(randomHour).padStart(2, '0');
        hourlyData[hour][sale.saleMethod] += salePrice;
      });
    });

    // 시간별 데이터를 결과 배열로 변환 (모든 시간대 포함)
    for (let i = 0; i < 24; i++) {
      const hour = String(i).padStart(2, '0');
      const hourData = hourlyData[hour];

      // 모든 판매 방법에 대해 데이터 포인트 생성 (0 값도 포함)
      Object.keys(hourData).forEach(method => {
        // 0 값도 명시적으로 포함
        result.push({
          createdAt: `2025-04-22 ${hour}:00`,
          orderType: method,
          totalPrice: hourData[method]
        });
      });
    }
  }
    ...
    <main class="main-content">
      ...
      <div class="sales-detail-container">
        <SalesDetail :selected-date="selectedDate" :sales-data="salesData" :selected-period="selectedPeriod"
          :chart-data="chartData" :date-range-type="dateRange.type" :is-loading="isLoading" />
      </div>
    </main>
  </div>
</template>
